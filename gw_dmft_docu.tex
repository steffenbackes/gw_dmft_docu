...
\section{Expressing $v$, $W$, $U$, $P=GG$ in a local basis}
...
\subsection{Definition of the local Polarization $P_{ijkl}$}
...
\subsection{Obtaining the local $U_{ab}$, $J_{ab}$ from $W_{abcd}$ and $P_{ijkl}$ }
...
However, in practice there are some simplifications.
...
We observe that the local projection $W_{loc}$ is significantly stronger screened 
than screening the cRPA bare interaction by the local RPA polarization 
$P_{\mathrm{loc}}=G_{\mathrm{loc}}G_{\mathrm{loc}}$. 
This difference is due to the fact that in the cRPA scheme the screening added or removed is based on
a band description, in contrast to the local screening calculated in a local orbital basis.
From this observation we can conclude that removing transitions between bands involves more
...
Is this the reason why Boehnke et. al obtain a smaller interaction?
...
\subsection{Complex term in $W$?}
To be checked:

It seems that the screening can make $W$ complex and it takes the form
\begin{align}
 W(i\nu_n)    &\approx w_0 + \frac{w_1}{i\nu_n} + \frac{w_2}{(i\nu_n)^2} .
\end{align}
Including this term, we have to calculate two additional contributions:
