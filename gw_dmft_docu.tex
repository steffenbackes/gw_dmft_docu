...
\begin{align}
 G(1,2)  
 &= \sum_{ij,p} \chi^p_{i}(r_1) G_{ij}(p,\tau_1,\tau_2) \chi^{p*}_{j}(r_2),
\end{align}
...
\begin{align}
 ...
 \, \mathrm{d}r_1 \mathrm{d}r_2 \\
%
&= \sum_{ ijkl, pp'} \int B^{q*}_{\alpha}(r_1) 
                        \chi^p_{i}(r_2) G_{ij}(p,\tau_2,\tau_3) \chi^{p*}_{j}(r_3) \nonumber \\
& \hspace{1.5cm}\times  \chi^{p'}_{k}(r_4) G_{kl}(p',\tau_4,\tau_2^+) \chi^{p'*}_{l}(r_2) \nonumber \\
& \hspace{1.5cm}	\times  \Gamma(3,4;1)  B^{q}_{\beta}(r_2) 
       \,  \mathrm{d}r_1 \mathrm{d}r_2\mathrm{d}(3)\mathrm{d}(4) .
\end{align}
From this expression we identify the matrix element of the 
...
&= \sum_{ ijkl,pp'} \int 
     \chi^{p'*}_{l}(r_2) \chi^p_{i}(r_2) B^{q}_{\beta}(r_2)
      G_{ij}(p,\tau_2,\tau_3) G_{kl}(p',\tau_4,\tau_2^+)  \nonumber \\
...
&= \sum_{ ijkl,p'} \int 
     M^{\beta*}_{li}(p',q) 
      G_{ij}(p'-q,\tau_2,\tau_3) G_{kl}(p',\tau_4,\tau_2^+)  \nonumber \\
...
&= \sum_{ ijkl,p} 
     M^{\alpha}_{kj}(p,q)
      G_{ij}(p-q,\tau_2,\tau_1) G_{kl}(p,\tau_1,\tau_2^+)
     M^{\beta*}_{li}(p,q).
\end{align}
and using time-translational invariance this takes the final form
\begin{align}
...
\begin{align}
 P_{\alpha\beta}(q,i\nu_n)
%
&= \sum_{ ijkl,p} \sum_{i\omega_n}
     M^{\alpha}_{kj}(p,q)
      G_{ij}(p-q,i\omega_n-i\nu_n) G_{kl}(p,i\omega_n)
     M^{\beta*}_{li}(p,q) \mathrm{e}^{-i\omega_n 0^+} \\
%
&=  \sum_{ ijkl,p} M^{\alpha}_{kj}(p,q)
    \left( \sum_{i\omega_n} G_{ij}(p-q,i\omega_n-i\nu_n) G_{kl}(p,i\omega_n) \mathrm{e}^{-i\omega_n 0^+}  \right)
...
\begin{align}
 G(1,4)  
 &= \sum_{ij,p} \chi^p_{i}(r_1) G_{ij}(p,\tau_1,\tau_4) \chi^{p*}_{j}(r_4),
\end{align}
and the screened interaction in the product basis as
\begin{align}
 W(1^+,3)  
...
&= \sum_{ij,p'}\sum_{\alpha\beta q}
  ...
  &\times \chi^{p'}_{i}(r_1) G_{ij}(p',\tau_1,\tau_4) \chi^{p'*}_{j}(r_4)
...
&= \sum_{ij}\sum_{\alpha\beta q}  M^{\alpha*}_{ai}(p,q)  W_{\alpha\beta}(q,\tau_1^+,\tau_2)  
                                  M^{\beta}_{bj}(p,q) G_{ij}(p-q,\tau_1,\tau_2) .
%&= \sum_{ijR_iR_j} W^{R_1R_jR_iR_2}_{ajib}(\tau_1^+,\tau_2)  G^{R_iR_j}_{ij}(\tau_1,\tau_2) .
\end{align}
...
we get
\begin{align}
\Sigma_{ab}(p,i\omega_n)
%
&= \sum_{ij,\alpha\beta}\sum_{q,ix_n}  M^{\alpha*}_{ai}(p,q)  W_{\alpha\beta}(q,i\nu_n)  
                                  M^{\beta}_{bj}(p,q)    \nonumber \\
& \hspace{1.5cm} \times G_{ij}(p-q,i\omega_n-i\nu_n) \mathrm{e}^{-i\nu_n0^+} .
\end{align}
