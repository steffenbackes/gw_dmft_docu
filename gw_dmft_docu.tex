...
\end{align}
If we now assume that all Green's functions in the equation above are generated using only a local Selfenergy,
then this equation can be simplified into a Dyson-like equation (see Rev.Mod.Phys.68,1(1996) for a detailed derivation)
\begin{align}
 \mathcal{G}^{-1} &= i\omega + \mu_{loc} -  \sum_{ij\neq 0} t_{0i}  \left( G_{ij} - \frac{G_{i0} G_{0j} }{G_{00}} \right) t_{j0} \label{eq:eff_dyson_for_bath} \\
 &= (G_{00})^{-1} + \Sigma.    \label{eq:wrong_dyson_for_bath}
\end{align}
It is very important to stress that Eq.\eqref{eq:wrong_dyson_for_bath} is only valid for 
a local Selfenergy.
If there are nonlocal Selfenergy terms present to generate the lattice Green's function 
like in GW+DMFT, this derivation is in general no longer possible
and one cannot obtain Eq.\eqref{eq:wrong_dyson_for_bath} from Eq.\eqref{eq:eff_dyson_for_bath} (a detailed derivation will be added later).
