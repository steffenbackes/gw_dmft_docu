...
%
 \int \frac{ f(x)x^2 }{y-x} \, \mathrm{dx} 
...
%
&= \left( i\omega + \mu -\Sigma(i\omega) \right) - \int D(\epsilon) \epsilon \, \mathrm{d}\epsilon - G_{loc}^{-1}.
\end{align}
So finally we obtain as the bath Green's function
\begin{align}
 \mathcal{G}^{-1} 
 &= G_{loc}^{-1} + \Sigma(i\omega) + \underbrace{ \left( \mu_{loc}- \mu + \int D(\epsilon) \epsilon \, \mathrm{d}\epsilon \right) }_{=0 \mbox{ when selfconsist.}} \\
