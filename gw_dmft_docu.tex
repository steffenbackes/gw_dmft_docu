...
\subsection{Evaluating the local polarization $P_{loc}$ to obtain $W_{loc}$}
\label{sec:local_polarization}
...
Therefore, $G^{\mathrm{loc}}=G^{\mathrm{DFT}}$.
...
Now we will describe how to evalute the local polarization and interaction
from the DFT impurity input via
\begin{align}
 P_{\mathrm{loc}} &= G_{\mathrm{loc}}G_{\mathrm{loc}} \\
 W_{\mathrm{loc}} &= [U^{-1}_{\mathrm{loc}} - P_{\mathrm{loc}} ]^{-1}.
\end{align}

We will start with the equation for the polarization in the
continuum and then project onto a local basis set
The polarization in the RPA approximation is given as
...
\begin{align}
 (1) &= (r_1,\sigma_1,t_1).
\end{align}
When comparing to the expression for $W$ (see Eq. (11) in Phys. Rev. 139, 3A (1965))
\begin{align}
 W(1,2)
 &= v(1,2)
  + \int  W(1,3) P(3,4) v(4,2) \, \mathrm{d}(3)\mathrm{d}(4),
\end{align}
we note that, due to $W$ and $v$ being spin-independent,
that the integral over spin simplifies to a summation of spin only on the polarization
\begin{align}
 W(1,2)
 &= v(1,2) \nonumber \\
  &+ \int  W(r_1,t_1;r_3,t_3) \nonumber \\
  &\times \sum_{\sigma_3\sigma_4} P(r_3,\sigma_3,t_3;r_4,\sigma_4,t_4)  \nonumber \\
  &\times v(r_4,t_4;r_2,t_2)\, \mathrm{d}(r_3,t_3)\mathrm{d}(r_4,t_4).
\end{align}
Therefore, the polarization used for the screening is the spin-summation over the 
spin-dependent polarization that we calculate from the
Green's functions.
...
obtain the matrix elements of the Polarization in this basis
\begin{align}
 \braket{m_1\sigma_1 | P(\tau_1,\tau_2) | m_2\sigma_2 } 
 &= \braket{\chi_{m_1}^{\sigma_1} | P(\tau_1,\tau_2) | \chi_{m_2}^{\sigma_2} }  \nonumber \\
 &= \int\int \mathrm{d}r_1\mathrm{d}r_2 \  \chi_{m_1}^{\sigma_1*}(r_1) 
                                 G(r_1,\sigma_1,\tau_1;r_2,\sigma_2,\tau_2) \nonumber \\
           & \hspace{3cm} \times G(r_2,\sigma_2,\tau_2;r_1,\sigma_1,\tau_1)\chi_{m_2}^{\sigma_2}(r_2).
\end{align}
...
\begin{align}
 \braket{m_1\sigma_1 | P(\tau_1,\tau_2) | m_2\sigma_2 } 
 &= \sum_{abcd}\int\int \mathrm{d}r_1\mathrm{d}r_2 \
                         \chi_{a}^{\sigma_1}(r_1) G^{\sigma_1\sigma_2}_{ab}(\tau_1,\tau_2)\chi_{b}^{\sigma_2*}(r_2) \nonumber \\
    &\hspace{1cm} \times \chi_{c}^{\sigma_2}(r_2) G^{\sigma_2\sigma_1}_{cd}(\tau_2,\tau_1)\chi_{d}^{\sigma_1*}(r_1)
\end{align}
...
\subsection{Relation between $U(i\nu_n)$, $W(i\nu_n)$ and $P(i\nu_n)$}
