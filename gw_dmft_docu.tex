...
GW Selfenergy $\Sigma^{GW}$ for all states. In addition, the local part of 
...
the selfconsistency is done including the full nonlocal effects of the combined 
Selfenergy.

No further selfconsistency apart from the DMFT cycle will be performed, 
{\it i.e.} no update of $\Sigma^{GW}$ will be done. By this, the final interacting 
...
for $\Sigma^{GW}$ is $i$ instead of $-1$. For $P$ I need to check...
...
\end{align}
\cng{Is this probably not exact}
...
slowes down the convergence even more. 
...
combination of 
\begin{align}
 \underbrace{H^{DFT}(k) - v^{XC}(k) - \Sigma^{Hartree}_{DFT}}_{\mbox{no exchange}}
          + \underbrace{\Sigma^{XC}_{GW}(k,i\omega_n) - \Sigma_{GW}^{DC}(i\omega_n)}_{\mbox{only nonlocal exch.}}
          + \underbrace{\Sigma_{imp}(i\omega_n)}_{\mbox{only local exch.}}
\end{align}
...
In the case of dynamical interactions $U(i\omega_n)$ in the Hartree and exchange part
the screened Coulomb matrix elements recover their bare values, i.e.
$U_0$ has to be replaced by the bare $V$ plus an additional term in the Hartree part
\begin{align}
 \Sigma^{Hartree}_{l\sigma}
&= -2K'(0^+)(N-1) + \sum_{m,\sigma'} \braket{lm|V_{bare}|lm} n^{\sigma'}_{m} \\
\Sigma^{Fock}_{l\sigma}
&= - \sum_{m}\braket{lm|V_{bare}|ml} n^{\sigma}_{m} 
\end{align}
Here, $N=\sum_{m\sigma} n_m^{\sigma}$ is the total local charge,
and $K'(\tau) = \partial_{\tau} K(\tau)$, as defined in section \ref{sec:gwdmft_cycle}.
$K'(0^+)$ is given as
\begin{align}
 K'(0^+)
...
$K(\tau)$ is not orbital dependent and depends only on the monopole term
\begin{align}
 K'(0^+)
 &= \frac{1}{2}\left[ F_0(\infty) - F_0(0) \right].
\end{align}
Usually we consider the monopole term $F_0$
...
form with the unscreened $F_0$ and $J$ values. If $J$ is not considered as
frequency dependent, like we do here, one has to choose whether to 
use the value at zero frequencies, the bare value or something in-between.
