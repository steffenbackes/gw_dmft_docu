...
to the action of an impurity model with the Weiss field
\begin{align}
...
\end{align}
This equation can be simplified assuming a local Selfenergy to obtain an effective Dyson equation
\begin{align}
 \mathcal{G}^{-1} &= i\omega + \mu_{loc} -  \sum_{ij\neq 0} t_{0i}  \left( G_{ij} - \frac{G_{i0} G_{0j} }{G_{00}} \right) t_{j0} \label{eq:eff_dyson_for_bath} \\
 &= (G_{00})^{-1} + \Sigma.
\end{align}
If we now assume a nonlocal Selfenergy, this derivation is in general no longer possible,
and the more general form of Eq.\eqref{eq:eff_dyson_for_bath} has to be used.

To obtain 

\subsubsection{2-site Dimer example}
For the case of a dimer with the two sites 0 and 1 the full Green's function is given by
\begin{align}
G^{-1} &=
...
&= i\omega + \mu_{loc} -  \frac{t^2}{i\omega + \mu-\Sigma_{00} } .
\end{align}
...
Here we consider a finite 4-site cluster with NN-Hopping $t=1$. We generate the bath 
and hybridization function for the generalized scheme and from the Dyson equation 
...
\subsubsection{2D Hubbard model example at the Fock term at half-filling}
Here we consider a 2-dimensional Hubbard model with NN-Hopping $t=1$. We generate the bath 
and hybridization function for the generalized scheme and from the Dyson equation 
and compare the results. Since the generalized scheme and the Dyson equation
are identical for local Selfenergies, we add here a Fock-like term as the nonlocal
Selfenergy of the form 
...
Then we perform a 2nd+DMFT calculation where the nonlocal Selfenergies are included in the DMFT cycle
...
We clearly see that the full 2nd+DMFT scheme is the clostest to the exact diagrammatic Monte Carlo
results. The second best result for the real part is the 2nd(w=0)+DMFT scheme, where we have approximated the 
nonlocal Selfenergy by their $w=0$ value. But interestingly the imaginary part is slightly too small,
indicating this methods leads to an underestimation of the correlation strength, i.e. likely at high energies, but 
gives a good representation at the Fermi level, since the real part of the Selfenergy is well reproduced.
The Dyson equation shows completely the opposite trend, regardless of using the frequency dependent
nonlocal Selfenergy or its $w=0$ value. Also here the Dyson equation leads to an underestimation
