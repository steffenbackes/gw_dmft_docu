...
\subsection{Representation of the $\Gamma \approx 1$ approximation
in a local basis}
...
Therefore, we will rederive the expression for the Selfenergy starting
...
\begin{align}
 G(1,4)  
 &= \sum_{ijR_iR_j} \chi_{iR_i}(r_1) G_{ij}(\tau_1,\tau_4) \chi^*_{jR_j}(r_4),
\end{align}
and the screened interaction in the product basis as
\begin{align}
 W(1^+,3)  
 &= \sum_{\alpha\beta} B_{\alpha}(r_1) W_{\alpha\beta}(\tau_1^+,\tau_3) B^*_{\beta}(r_3).
\end{align}
...
%
&= \sum_{ijR_iR_j}\sum_{\alpha\beta}
   \int \chi^*_{aR_1}(r_1) B_{\alpha}(r_1) W_{\alpha\beta}(\tau_1^+,\tau_3) B^*_{\beta}(r_3) \nonumber\\
  &\times \chi_{iR_i}(r_1) G_{ij}(\tau_1,\tau_4) \chi^*_{jR_j}(r_4)
...
vertex as
\begin{align}
\Gamma^{R_jR_2}_{jb,\beta}(\tau_4,\tau_2;\tau_3)
&= \int \chi^*_{jR_j}(r_4) \chi_{bR_2}(r_2) \Gamma(4,2;3) B^*_{\beta}(r_3)
