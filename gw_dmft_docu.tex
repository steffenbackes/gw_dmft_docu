...
therefore it is better to put the frequency shift in $W(i\nu)$,
...
\textbf{This term contains a Hartree part, which should not be incorporated, 
...
In general, the Hartree term from the local GW Selfenergy should be identical
...
Therefore, the two Hartree correction terms just cancel out
\begin{align}
 G(k,i\omega_n) 
...
from DFT. Therefore, it does not need to be calculated explicitly.
...
We adopt the convention that the GW Doublecounting $\Sigma^{GW,imp}(i\omega_n)$
will always contain the Hartree part from DFT regardless of how the exchange-correlation
part of it is determined. Therefore, this term takes care of the Hartree Doublecounting
from DFT$\leftrightarrow \Sigma^{imp}$ and also of the dynamic Doublecounting between
$\Sigma^{GW}\leftrightarrow \Sigma^{imp}$.
