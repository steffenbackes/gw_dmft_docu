...
\section{Noncausal bath and ED+DMFT for a dimer}
...
\subsection{Analytic form of the bath for the dimer}
...
\subsection{Generalized DMFT equations with a nonlocal Selfenergy}
In the derivation of the DMFT equations using the cavity construction one can show that the
effective action of the Hubbard model after integrating out all sites except one becomes identical
to the action of an impurity model with the Weiss field (all objects here are matrices in orbital space)
\begin{align}
 \mathcal{G}^{-1} = i\omega + \mu_{loc} - \sum_{ij\neq 0} t_{0i} G^{(0)}_{ij} t_{j0},
\end{align}
where $G^{(0)}_{ij}$ is the Green's function of the Hubbard model with site $0$ removed, and
$t_{i0}$ are the hopping amplitudes from site $i$ to the site $0$.

For a general lattice the relation between the cavity $G^{(0)}$ and the full lattice Green's function $G$ reads
\begin{align*}
 G^{(0)}_{ij} &= G_{ij} - \frac{G_{i0} G_{0j} }{G_{00}},
\end{align*}
so the effective Weiss field takes the form
\begin{align}
...
%
&= \left( i\omega + \mu -\Sigma(i\omega) \right) - \underbrace{\int D(\epsilon) \epsilon \, \mathrm{d}\epsilon}_{\int \epsilon(k) \, \mathrm{d}k} - G_{loc}^{-1}.
\end{align}
So finally we obtain as the bath Green's function
\begin{align}
 \mathcal{G}^{-1} 
 &= G_{loc}^{-1} + \Sigma(i\omega) + \underbrace{ \left( \mu_{loc}- \mu
      + \int \epsilon(k) \, \mathrm{d}k \right) }_{=0 \mbox{ when selfconsist.}} \\
 &= G_{loc}^{-1} + \Sigma(i\omega).
\end{align}
So we obtain a Dyson-like equation for the bath.
...
like in GW+DMFT, this derivation is in general no longer possible because the integration over momentum
space cannot be replaced by an integration over energy and the DOS.}
...
\begin{align}
 \mathcal{G}^{-1} &= i\omega + \mu_{loc} -  t_{01} t_{10} \left( G_{11} - \frac{G_{10} G_{01} }{G_{00}} \right) \\
 &= i\omega + \mu_{loc} -  \frac{t^2}{(i\omega + \mu-\Sigma_{00})^{2} -(t-\Sigma_{01})^2 } 
                 \left( i\omega + \mu-\Sigma_{00} - \frac{ (-t+\Sigma_{01})^2 }{i\omega + \mu-\Sigma_{00}} \right) \\
&= i\omega + \mu_{loc} -  \frac{t^2}{i\omega + \mu-\Sigma_{00} } . \label{eq:correct_bath_dimer}
\end{align}
...
\subsubsection{Multi-orbital generalization}
...
\subsection{Generalized EDMFT equations for $U(i\nu)$ with nonlocal polarization}
In the same way we derived the effective bath and hybridization with nonlocal Selfenergies,
we now derive the equations for the effective impurity interaction $U$.
...
\begin{align}
G^{-1}(k,i\omega) &= (\underbrace{i\omega+\mu-\epsilon(k)}_{G_0^{-1}}) - \Sigma(k,i\omega) \\
W^{-1}(q,i\nu) &= v^{-1}(q)-P(q,i\nu) .
\end{align}
For an extened Hubbard model with nonlocal interactions it has been shown by Qimiao Si and J. Lleweilun Smith in 
PRL 77, 16 (1996) that the density-density correlation function $\chi(q,i\nu)$ obeys the same relations 
as the Green's function in a cavity construction
\begin{align}
 \Delta(i\omega) &= -\sum_{ij\neq 0} t_{0i} G^{(0)}_{ij}(i\omega) t_{j0} \\
 \Theta^{-1}(i\nu) &= \sum_{ij\neq 0} v_{0i} \chi^{(0)}_{ij}(i\nu) v_{j0} ,
\end{align}
where $\Theta$ is the bosonic bath for the interaction.
Since the relation for the effective impurity interaction is
\begin{align}
 U(i\nu) &= \Theta^{-1}(i\nu) + P...................
\end{align}
The corresponding objects on a lattice without site $0$ are given by
\begin{align}
 G^{(0)}_{ij} &= G_{ij} - G_{i0}G^{-1}_{00}G_{0j} \\ 
 \chi^{(0)}_{ij} &= \chi_{ij} - \chi_{i0}\chi^{-1}_{00}\chi_{0j} .
\end{align}
We see that the bare interaction $v$ plays the same role as the noninteracting hopping amplitude $t$.
...
\boxed{
 \Theta^{-1}(i\nu) 
 = \braket{ v \chi v } - \braket{v \chi} \braket{\chi}^{-1} \braket{\chi v}
 }. \label{eq:eff_dyson_for_interaction}
\end{align}
In case of a purely local polarization $P(q,i\nu)=P(i\nu)$, we can derive again a DMFT-like Dyson equation:
We consider a single orbital model and assume that the integral transformation
\begin{align}
 \int v(q)  \, \mathrm{d}q &= \int D(v) v  \, \mathrm{d}v,
\end{align}
exists. Furthermore, we use the relations between the susceptibility and polarization
\begin{align}
 \chi(q,i\nu) &= \frac{1}{v(q) - P^{-1}(q,i\nu) } \\
 v(q) &= \frac{1}{\chi(q,i\nu)} + \frac{1}{P(q,i\nu)} \\
 \chi(q,i\nu) &= -P(q,i\nu) - P(q,i\nu)W(q,i\nu)P(q,i\nu).
\end{align}
With this we get
\begin{align}
  \Theta^{-1}(i\nu) 
 &= \braket{ v \chi v } - \braket{v \chi} \braket{\chi}^{-1} \braket{\chi v} \nonumber \\
 &= \int \frac{D(v) v^2}{v-P^{-1}} \, \mathrm{d}v
    - \frac{1}{\chi_{loc}} \left( \int \frac{D(v) v}{v-P^{-1}} \, \mathrm{d}v \right)^2 \nonumber \\
 %
 &= \frac{\chi_{loc}}{P^2} + \frac{1}{P} + \int D(v) v  \, \mathrm{d}v 
    -\frac{1}{\chi_{loc}} \left( \frac{\chi^2_{loc}}{P^2} + 1 + 2\frac{\chi_{loc}}{P} \right) \nonumber \\
%
&= \underbrace{\int v(q)  \, \mathrm{d}q}_{ [1/\chi]_{loc}+1/P} - \frac{1}{P} - \frac{1}{\chi_{loc}} \nonumber \\
%
&\boxed{ = \left[ \frac{1}{\chi} \right]_{loc} - \frac{1}{\chi_{loc}} } \\
%
&= \int v(q)  \, \mathrm{d}q - \frac{1}{P} + \frac{1}{ P + PW_{loc}P } \nonumber \\
&= \int v(q)  \, \mathrm{d}q - \frac{1}{P}\left( 1 - \frac{1}{ 1 + PW_{loc} } \right) \nonumber \\
&= v_{loc} - \frac{1}{W^{-1}_{loc}+P} \nonumber \\
\end{align}
Then?
\begin{align}
 U(i\nu) &= v_{loc} - \Theta^{-1}(i\nu) \nonumber  \\
 &= \left[ W^{-1}_{loc} + P \right]^{-1}.
\end{align}
