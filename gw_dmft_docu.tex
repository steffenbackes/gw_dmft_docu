...
because the poles of $G_0( z - i\nu_n)$ would be located in the complex plane
and not on the real axis.
...
has discontinuos poles on the real axis coming from above/below.
Therefore, we separate the convolution
\begin{align}
 F(i\omega_n)  &= \frac{1}{\beta} \sum_{i\nu_n\neq 0} B(i\nu_n) G_0(i\omega_n - i\nu_n) 
...
Therefore, $B(i\delta) \neq B(0)$, since $B$ can have a delta-like peak at the origin.
This leads to
\begin{align}
 F(i\omega_n)
...
\end{align}
We still cannot do $i\omega_n \rightarrow \omega + i\delta$ since $n_B(i\omega_n-y)$
has poles in the complex plane. But we can evaluate it first
\begin{align}
...
\end{align}
In this form all poles are on the real axis and we can safely evaluate on the real axis.
...
\end{align}
It is important to note that the delta-peak in $B(x)$ at $x=0$ does not enter the integral
in the first two terms, because we evaluate $B$ slightly above the real axis $B(x+i\delta)$.
...
\begin{align}
 ...
 &=\frac{1}{\pi} \int \frac{\mathrm{Im}B(x) A_0(\omega - x)}{ \mathrm{e}^{\beta x} - 1 } \mathrm{d}x 
  +\frac{1}{\pi} \int \mathrm{Im}\frac{B(x) A_0(\omega - x) }{\mathrm{e}^{-\beta (\omega - x)} + 1 } \mathrm{d}x \nonumber \\
...
& \hspace{1cm} + \frac{1}{\beta} A_0(\omega) \mathrm{Re}\left[ B(0) - B(i\delta) \right] ,
\end{align}
where $A_0= -\frac{1}{\pi} \mathrm{Im}G_0$.
