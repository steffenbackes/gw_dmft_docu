...
\begin{align}
 ...
 %
%  &\neq  \sum_{\substack{k_1,n_1n_2 \\ k_2,n_3, n_4}}  M^{i}_{n_1n_2}(k_1,q) M^{j*}_{n_3n_4}(k_2,q)
%  \  \braket{\psi^{k_1}_{n_1} | \psi_{n_3}^{k_2}} \  \braket{\psi_{n_4}^{k_2-q} |\psi_{n_2}^{k_1-q}} .
 %
\end{align}
...
&\hspace{1cm} + \sum_{i} \int_0^{\beta} \mathrm{d}\tau \ n_{i\uparrow}(\tau) v_{ii} n_{i\downarrow}(\tau)
+ \sum_{i<j} \int_0^{\beta} \mathrm{d}\tau \ n_{i}(\tau) v_{ij} n_{j}(\tau),
\end{align}
where $c_{i\sigma}(\tau),c^*_{i\sigma}(\tau)$ are anticommuting Grassman variables, 
$n_{i\sigma}(\tau) = c^*_{i\sigma}(\tau)c_{i\sigma}(\tau)$ and $n_{i}(\tau) = \sum_{\sigma} n_{i\sigma}(\tau)$.
...
&= \int_0^{\beta} \mathrm{d}\tau \sum_{\sigma} c^*_{0\sigma}(\tau)\left[ \frac{\partial}{\partial \tau} - \mu \right] c_{0\sigma}(\tau) 
 ...
 &\hspace{1cm}+  \int_0^{\beta} \mathrm{d}\tau \ n_{0\uparrow}(\tau) v_{00} n_{0\downarrow}(\tau).
\end{align}
...
&\hspace{1cm} + \sum_{j\neq 0 } \int_0^{\beta} \mathrm{d}\tau \ n_{0}(\tau) v_{0j} n_{j}(\tau).
\end{align}
...
&\hspace{1cm} + \sum_{i\neq 0 } \int_0^{\beta} \mathrm{d}\tau \ n_{i\uparrow}(\tau) v_{ii} n_{i\downarrow}(\tau)
+ \sum_{\substack{i<j \\ ij\neq 0  }} \int_0^{\beta} \mathrm{d}\tau \ n_{i}(\tau) v_{ij} n_{j}(\tau).
\end{align}
...
The first order terms also vanish because it contains only expection values of one Grassman variable,
and the $v_{ij}$ term is of order $1/d$.
...
&= \int_0^{\beta} \mathrm{d}\tau \sum_{\sigma} c^*_{0\sigma}(\tau)\left[ \frac{\partial}{\partial \tau} - \mu + t_{00} \right] c_{0\sigma}(\tau) 
    + \int_0^{\beta} \mathrm{d}\tau \ v_{00} n_{0\uparrow}(\tau)n_{0\downarrow}(\tau)  \nonumber \\
...
&\hspace{1cm}- \sum_{ij\neq 0} v_{0i} v_{j0}  \int_0^{\beta} \mathrm{d}\tau_1 \int_0^{\beta} \mathrm{d}\tau_2 \ n_{0}(\tau_1) \braket{ n_{i}(\tau_1)   n_{j}(\tau_2) }_{(0)}  n_{0}(\tau_2)  .
\end{align}
%
% \begin{align}
%  \sum_{ij}\int t_{0i} c^*_0(\tau_1) c_i(\tau_1)  \int t_{j0} c^*_j(\tau_2) c_0(\tau_2) 
% &= \sum_{ij}\int t_{0i}t_{j0} c^*_0(\tau_1) \Big[ c_i(\tau_1)   c^*_j(\tau_2) \Big] c_0(\tau_2) 
% \end{align}
% \begin{align}
%  \chi(\nu) &= \int_0^\beta \chi(\tau) e^{i \nu \tau} d\tau \\
%  &= \chi_0  \int_0^\beta  e^{i \nu \tau} d\tau \\
%  &= \chi_0  \frac{1}{i\nu}\Big[ e^{i \nu \tau} \Big]_0^\beta \\
%  &= \chi_0  \frac{1}{i\nu}\Big[ e^{i 2n \pi } - 1 \Big] \\
%  &= -\chi_0  \frac{1}{i\nu} \\
%  &=  i   \frac{\chi_0}{\nu}.
% \end{align}
...
\begin{align}
 ...
 \chi_{ij}^{(0)}(\tau_1-\tau_2) &= \braket{ n_{i}(\tau_1)   n_{j}(\tau_2) }_{(0)} ,
\end{align}
...
&\hspace{1cm}    + \int_0^{\beta} \mathrm{d}\tau_1 \int_0^{\beta} \mathrm{d}\tau_2 \ n_{0}(\tau_1) 
       \left( \delta(\tau_1-\tau_2) v_{00} - \sum_{ij\neq 0} v_{0i} v_{j0} \chi^{(0)}_{ij}(\tau_1-\tau_2) \right)  n_{0}(\tau_2)  .
\end{align}
...
\boxed{
 \mathscr{G}^{-1} (i\omega)
%  &= i\omega + \mu_{loc} - \left( [\epsilon G \epsilon]_{loc} - [\epsilon G]_{loc} [G_{loc}]^{-1}  [G \epsilon]_{loc}  \right).\\
...
% For simplicity, we now focus on the one-orbital case. The two last terms in the above equation take the form
% \begin{align}
%  \braket{ \epsilon G \epsilon } - \braket{\epsilon G} \braket{G}^{-1}  \braket{ G \epsilon } 
%  &= \int \frac{\epsilon^2(k)}{i\omega + \mu - \epsilon(k)-\Sigma(k,i\omega)}\, \mathrm{d}k \nonumber \\
%  & \hspace{0.5cm} - \frac{1}{G_{loc}} \left( \int \frac{\epsilon(k)}{i\omega + \mu - \epsilon(k)-\Sigma(k,i\omega)} \, \mathrm{d}k \right)^2 
% \end{align}
% \underline{If we now neglect the momentum-dependence of the Selfenergy} $\Sigma(k,i\omega)\approx \Sigma(i\omega)$ we can substitute
% the integration over $k$ by an integration over energy and the DOS
% \begin{align}
%  \braket{ \epsilon G \epsilon } - \braket{\epsilon G} \braket{G}^{-1}  \braket{ G \epsilon } 
%  &= \int \frac{ D(\epsilon) \epsilon^2}{i\omega + \mu - \epsilon-\Sigma(i\omega)} \, \mathrm{d}\epsilon \nonumber \\
%  & \hspace{0.5cm} - \frac{1}{G_{loc}} \left( \int \frac{ D(\epsilon) \epsilon}{i\omega + \mu - \epsilon-\Sigma(i\omega)} \mathrm{d}\epsilon \right)^2.
% \end{align}
% We can simplify this equation by  using the following mathematical identities
% \begin{align}
% \int \frac{ f(x)x }{y-x} \, \mathrm{dx} &= y\int \frac{ f(x) }{y-x} \, \mathrm{dx} - 1 \\
% %
%  \mbox{and }\int \frac{ f(x)x^2 }{y-x} \, \mathrm{dx} 
%  &= y\int \frac{ f(x)x }{y-x} \, \mathrm{dx} - \int f(x) x \, \mathrm{dx} \nonumber \\
% &= y^2 \int \frac{ f(x) }{y-x} \, \mathrm{dx} -y - \int f(x) x \, \mathrm{dx} .
% \end{align}
% With this we obtain
% \begin{align}
%  \braket{ \epsilon G \epsilon } - \braket{\epsilon G} \braket{G}^{-1}  \braket{ G \epsilon } 
%  &= \left( i\omega + \mu -\Sigma(i\omega) \right)^2 \int \frac{ D(\epsilon) }{i\omega + \mu - \epsilon-\Sigma(i\omega)} \, \mathrm{d}\epsilon \nonumber \\
%  & \hspace{0.5cm}- \left( i\omega + \mu -\Sigma(i\omega) \right) - \int D(\epsilon) \epsilon \, \mathrm{d}\epsilon \nonumber \\
%  & \hspace{0.5cm} - \frac{1}{G_{loc}} \left( \left( i\omega + \mu -\Sigma(i\omega) \right) \int \frac{ D(\epsilon) }{i\omega + \mu - \epsilon-\Sigma(i\omega)} \, \mathrm{d}\epsilon -1 \right)^2 \\
% %
% &= - \left( i\omega + \mu -\Sigma(i\omega) \right) - \int D(\epsilon) \epsilon \, \mathrm{d}\epsilon \nonumber \\
% &\hspace{0.5cm}   + 2\left( i\omega + \mu -\Sigma(i\omega) \right) - G_{loc}^{-1} \\
% %
%  &= \left( i\omega + \mu -\Sigma(i\omega) \right) - \underbrace{\int D(\epsilon) \epsilon \, \mathrm{d}\epsilon}_{\braket{\epsilon(k)}} - G_{loc}^{-1}.
% \end{align}
% So finally we obtain as the bath Green's function
% \begin{align}
%  \mathscr{G}^{-1}(i\omega)
%  &= i\omega + \mu - \braket{\epsilon} -\Big( 
%        i\omega + \mu -\Sigma(i\omega)  - \braket{\epsilon} - G_{loc}^{-1} 
%         \Big) \\
...
% Here, the Polarization $P(q,i\nu)$ takes the role of the nonlocal Selfenergy $\Sigma(k,i\omega)$,
% which dresses the bare interaction $v(q)$ to the screened interaction $W(q,i\nu)$, i.e.
% \begin{align}
% G^{-1}(k,i\omega) &= \underbrace{G_0^{-1}}_{i\omega+\mu-H(k)} - \Sigma(k,i\omega) \\
% W^{-1}(q,i\nu) &= v^{-1}(q)-P(q,i\nu) .
% \end{align}
Previously we obtained the expression for the effective local interaction as 
\begin{align}
 U(i\nu) 
 &= v_{00} - \underbrace{\sum_{ij\neq 0} v_{0i} v_{j0} \chi_{ij}(i\nu)}_{\Theta(i\nu)}.
\end{align}
...
% \includegraphics[width=1.0\textwidth]{figs/reversebath/dimerbath/bath_general_dyson_U2.pdf}
...
\begin{align}
 ...
 \begin{align}
 \mathscr{G}^{-1}(i\omega) 
 &=i\omega + \braket{\epsilon} - \sum_{ij\neq 0 } t_{0i} G_{ij}^{(0)} t_{j0} \\
 &=i\omega + \braket{\epsilon} - \sum_{ij\neq 0 } t_{0i} \left( G_{ij} - G_{i0}G^{-1}_{00} G_{0j} \right) t_{j0} \\
...
%
 ...
 \end{align}
We now apply probing fields coupling to the fermionic propagators
\begin{align}
 S_{eff}[c,c^*,J_{G},J_{\epsilon G}, J_{G \epsilon},J_{\epsilon G \epsilon}]
 &= S_{eff}[c,c^*] 
  - \int\mathrm{d}\tau\mathrm{d}\tau' c^*_0(\tau) J_{G}(\tau,\tau') c_0(\tau') \nonumber \\
  &\hspace{2cm} - \int\mathrm{d}\tau\mathrm{d}\tau' \sum_{i\neq 0} t_{0i} c^*_i(\tau) J_{\epsilon G}(\tau,\tau') c_0(\tau')\nonumber \\
  &\hspace{2cm} - \int\mathrm{d}\tau\mathrm{d}\tau' \sum_{i\neq 0} c^*_0(\tau) J_{G \epsilon}(\tau,\tau') c_i(\tau') t_{i0}\nonumber \\
  &\hspace{2cm} - \int\mathrm{d}\tau\mathrm{d}\tau' \sum_{ij\neq 0} t_{0i}c^*_i(\tau) J_{\epsilon G \epsilon}(\tau,\tau') c_j(\tau') t_{j0}.
\end{align}
The free energy is
\begin{align}
...
\end{align}
from which the local propagators can be derived as
\begin{align}
 \braket{G}(\tau,\tau') &= G_{00} = \frac{\delta F}{\delta J_{G}} = -\braket{T  c_0(\tau) c^*_0(\tau') } \\
 \braket{\epsilon G}(\tau,\tau') &= \frac{\delta F}{\delta J_{ \epsilon G}} = - \sum_{i\neq 0} t_{0i} \braket{T  c_i(\tau)c^*_0(\tau') } \\
 \braket{G \epsilon}(\tau,\tau') &= \frac{\delta F}{\delta J_{G \epsilon}} = - \sum_{i\neq 0}  \braket{T  c_0(\tau)c^*_i(\tau') }t_{i0} \\
 \braket{\epsilon G \epsilon}(\tau,\tau') &= \frac{\delta F}{\delta J_{\epsilon G \epsilon}} = - \sum_{ij\neq 0} t_{i0} \braket{T  c_i(\tau)c^*_j(\tau') } t_{j0} .
\end{align}
Then we perform a Legendre transform of $F$ to obtain the functional
\begin{align}
 \Gamma[\braket{G}, \braket{\epsilon G}, \braket{G \epsilon }, \braket{\epsilon G \epsilon}]
 &=F[J_{G},J_{\epsilon G}, J_{G \epsilon},J_{\epsilon G \epsilon}] 
 - J_{G} \braket{G} \nonumber \\
 &\hspace*{2cm} -  J_{\epsilon G} \braket{\epsilon G} -  J_{G \epsilon} \braket{G\epsilon}
 -  J_{\epsilon G \epsilon} \braket{\epsilon G \epsilon}.
\end{align}
This functional now has the properties
\begin{align}
  J_{G} &= \frac{\delta \Gamma}{\delta \braket{G}} \\
  J_{\epsilon G} &= \frac{\delta \Gamma}{\delta \braket{\epsilon G}} \\
  J_{G \epsilon} &= \frac{\delta \Gamma}{\delta \braket{G \epsilon}} \\
  J_{\epsilon G \epsilon} &= \frac{\delta \Gamma}{\delta \braket{\epsilon G \epsilon}}.
\end{align}
I think the first term $J_{G}$ can be identified with the contributions to the local Selfenergy arising
only from local propagators $\braket{G}$, so it is not equal to $\braket{\Sigma}$ yet.
But the other terms look like they correspond to local Selfenergy terms arising from nonlocal propagators $G_{0i}, G_{i0}$
and $G_{ij}$, weighted by the hopping factors...?
